...
class HTTPClient:
    ...
    async def request(
        ...
        *,
        ...
        resp = await self.session.request(
            method, f"/api/v{self.DISCORD_API_VERSION}{path}",
            params=params,
    ...
    async def sync_global_commands(self, application_id: str, commands: List[Dict[str, Any]]):
        return await self.request("PUT", f"/applications/{application_id}/commands", json=commands, authorize=True)

    async def sync_guild_commands(self, application_id: str, guild_id: str, commands: List[Dict[str, Any]]):
        return await self.request(
            "PUT", f"/applications/{application_id}/guilds/{guild_id}/commands", json=commands, authorize=True)

    async def fetch_global_application_commands(self, application_id: str):
        return await self.request("GET", f"/applications/{application_id}/commands", authorize=True)
    ...
    async def delete_command(self, application_id: str, command_id: str, guild_id: Optional[str] = None):
        if guild_id:
            return await self.request(
                "DELETE", f"/applications/{application_id}/guilds/{guild_id}/commands/{command_id}", authorize=True)
        return await self.request("DELETE", f"/applications/{application_id}/commands/{command_id}", authorize=True)

    async def send_message(self, channel_id: str, form: aiohttp.MultipartWriter):
        return await self.request("POST", f"/channels/{channel_id}/messages", form=form, authorize=True)

    async def create_dm_channel(self, payload: Dict[str, Any]):
        return await self.request("POST", "/users/@me/channels", json=payload, authorize=True)
    ...
    async def fetch_channel_message(self, channel_id: str, message_id: str):
        return await self.request("GET", f"/channels/{channel_id}/messages/{message_id}", authorize=True)

    async def fetch_channel_messages(self, channel_id: str, params: Dict[str, Any]):
        return await self.request("GET", f"/channels/{channel_id}/messages", params=params, authorize=True)

    async def delete_channel_message(self, channel_id: str, message_id: str):
        await self.request("DELETE", f"/channels/{channel_id}/messages/{message_id}", authorize=True)

    async def delete_channel_messages(self, channel_id: str, payload: Dict[str, Any]):
        await self.request("POST", f"/channels/{channel_id}/messages/bulk-delete", json=payload, authorize=True)

    async def pin_channel_message(self, channel_id: str, message_id: str):
        await self.request("PUT", f"/channels/{channel_id}/messages/{message_id}/pin", authorize=True)

    async def unpin_channel_message(self, channel_id: str, message_id: str):
        await self.request("DELETE", f"/channels/{channel_id}/messages/{message_id}/pin", authorize=True)
    ...
    async def edit_webhook_message(
        ...
        return await self.request("PATCH", f"/webhooks/{webhook_id}/{webhook_token}/messages/{message_id}", form=form)
    ...
    async def add_role(self, guild_id: str, user_id: str, role_id: str, *, reason: Optional[str] = None):
        return await self.request(
            "PUT", f"/guilds/{guild_id}/members/{user_id}/roles/{role_id}", reason=reason, authorize=True)

    async def remove_role(self, guild_id: str, user_id: str, role_id: str):
        return await self.request("DELETE", f"/guilds/{guild_id}/members/{user_id}/roles/{role_id}", authorize=True)
    ...
    async def kick_user(self, guild_id: str, user_id: str):
        return await self.request("DELETE", f"/guilds/{guild_id}/members/{user_id}", authorize=True)
    ...
    async def send_interaction_mp_callback(
        ...
        return await self.request("POST", f"/interactions/{interaction_id}/{interaction_token}/callback", form=form)

    async def fetch_guild(self, guild_id: str):
        return await self.request("GET", f"/guilds/{guild_id}?with_counts=true", authorize=True)

    async def fetch_guild_member(self, guild_id: str, user_id: str):
        return await self.request("GET", f"/guilds/{guild_id}/members/{user_id}", authorize=True)
    ...
    async def create_guild_channel(self, guild_id: str, payload: Dict[str, Any]):
        return await self.request("POST", f"/guilds/{guild_id}/channels", json=payload, authorize=True)

    async def crosspost_channel_message(self, channel_id: str, message_id: str):
        return await self.request("POST", f"/channels/{channel_id}/messages/{message_id}/crosspost", authorize=True)

    async def edit_channel(self, channel_id: str, payload: Dict[str, Any]):
        return await self.request("PATCH", f"/channels/{channel_id}", json=payload, authorize=True)

    async def edit_guild_channel_position(self, guild_id: str, payload: Dict[str, Any]):
        return await self.request("PATCH", f"/guilds/{guild_id}/channels", json=payload, authorize=True)

    async def create_guild_role(self, guild_id: str, payload: Dict[str, Any]):
        return await self.request("POST", f"/guilds/{guild_id}/roles", json=payload, authorize=True)

    async def edit_guild_role_position(self, guild_id: str, payload: Dict[str, Any]):
        return await self.request("PATCH", f"/guilds/{guild_id}/roles", json=payload, authorize=True)

    async def edit_guild_role(self, guild_id: str, role_id: str, payload: Dict[str, Any]):
        return await self.request("PATCH", f"/guilds/{guild_id}/roles/{role_id}", json=payload, authorize=True)

    async def create_guild_emoji(self, guild_id: str, payload: Dict[str, Any]):
        return await self.request("POST", f"/guilds/{guild_id}/emojis", json=payload, authorize=True)

    async def create_webhook(self, channel_id: str, payload: Dict[str, Any]):
        return await self.request("POST", f"/channels/{channel_id}/webhooks", json=payload, authorize=True)

    async def execute_webhook(
            self, webhook_id: str, webhook_token: str, form: aiohttp.MultipartWriter, params: Dict[str, Any]):
        return await self.request("POST", f"/webhooks/{webhook_id}/{webhook_token}", form=form, params=params)

    async def edit_webhook(self, webhook_id: str, payload: Dict[str, Any]):
        return await self.request("PATCH", f"/webhooks/{webhook_id}", json=payload, authorize=True)
    ...
    async def create_message_reaction(self, channel_id: str, message_id: str, emoji: str):
        return await self.request(
            "PUT", f"/channels/{channel_id}/messages/{message_id}/reactions/{emoji}/@me", authorize=True)

    async def delete_message_reaction(self, message_id: str, emoji: str, user_id: str):
        return await self.request(
            "DELETE", f"/channels/{message_id}/messages/{message_id}/reactions/{emoji}/{user_id}", authorize=True)
    ...
    async def create_test_entitlement(self, application_id: str, payload: Dict[str, Any]):
        return await self.request("POST", f"/applications/{application_id}/entitlements", json=payload, authorize=True)

    async def delete_test_entitlement(self, application_id: str, entitlement_id: str):
        return await self.request(
            "DELETE", f"/applications/{application_id}/entitlements/{entitlement_id}", authorize=True)

    async def fetch_entitlement(self, application_id: str, entitlement_id: str):
        return await self.request(
            "GET", f"/applications/{application_id}/entitlements/{entitlement_id}", authorize=True)

    async def fetch_entitlements(self, application_id: str, params: Dict[str, Any]):
        return await self.request(
            "GET", f"/applications/{application_id}/entitlements", params=params, authorize=True)
